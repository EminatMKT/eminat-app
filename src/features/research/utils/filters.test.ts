...
describe('filtros de leads (NCT# y rango de carga)', () => {
  ...
  it('acota por rango de fecha de carga, en una sola clave', () => {
    expect(ncts({ added: '2026-08-05..' })).toEqual(['NCT00000222'])
    expect(ncts({ added: '..2026-08-05' })).toEqual(['NCT00000111'])
    expect(ncts({ added: '2026-08-01..2026-08-31' })).toEqual(['NCT00000111', 'NCT00000222'])
  })

  it('deja fuera del rango a los leads sin fecha de carga', () => {
    expect(ncts({ added: '2000-01-01..' })).not.toContain('NCT00000333')
  })
})
