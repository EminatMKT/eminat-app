'use client'
import { useApp, ESTADO_COLORS, COLUMNAS_KANBAN, mesATrimestre } from '@/shared/context/AppContext'
...
export default function ActivityDetailModal() {
  const { t } = useT()
  const { s1, s2, border, accent, t1, t2, t3, setActividades, mostrarMensaje, miembrosPorId, colorMarca } = useApp()
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
          <div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 8, background: `${ESTADO_COLORS[modalVerAct.estado] || t3}20`, color: ESTADO_COLORS[modalVerAct.estado] || t3 }}>{estadoLabel(modalVerAct.estado, t)}</span>
            </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {COLUMNAS_KANBAN.map(col => (
              ...
              }} style={{ padding: '6px 14px', borderRadius: 20, fontSize: 12, border: `2px solid ${modalVerAct.estado === col ? ESTADO_COLORS[col] : border}`, background: modalVerAct.estado === col ? `${ESTADO_COLORS[col]}20` : 'transparent', color: modalVerAct.estado === col ? ESTADO_COLORS[col] : t2, cursor: 'pointer', fontWeight: modalVerAct.estado === col ? 700 : 400 }}>{estadoLabel(col, t)}</button>
            ))}
          </div>
        </div>
