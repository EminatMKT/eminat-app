...
- [x] **[Infra] Entornos en 3 tiers (local | development | production)** — Refactor del modelo de entornos a 3 tiers vía `APP_ENV`: `local` (Supabase local), `development` (Vercel Preview + dev remoto `ydcadspinryybextlvyi`), `production` (Vercel main + prod `ruedelunbtaomhrzgelc`). Deriva `isProdDb`/`isDevDb` (badge "DEV") + `superRefine` que rompe el build si un tier no-production apunta a la base de prod; `.gitignore` endurecido (todos los `.env*` salvo `.env.example`). _(creado por: EminatMKT · 2026-06-26)_ ✓ _resuelto: commits 1778a4b, bcf9107, 22021e1; PR #20→development y PR #21→main (en prod). El bug client-side (la validación corría en el browser leyendo la var sin prefijo `NEXT_PUBLIC_` → `undefined`→`local` → `ZodError` solo en prod) se corrigió renombrando a `NEXT_PUBLIC_APP_ENV`; hotfix PR #22 (pendiente de merge + setear `NEXT_PUBLIC_APP_ENV=production` en Vercel Production) — responsable: EminatMKT · 2026-06-29T19:38-05:00_
