...
export default function AppShell({ children, title, actions, activeTab, onTabChange }: Props) {
  ...
  const { usuario, dark, setDark, horaActual, onlineCount, mensaje, notificaciones, notifAbiertas, setNotifAbiertas, setNotificaciones, accent, cargo, role, handleLogout, bg, s1, s2, border, t1, t2, t3 } = app
        ...
        <div style={{ width: panelOpen ? 172 : 0, background: s1, borderRight: panelOpen ? `1px solid ${border}` : 'none', overflow: 'hidden', transition: 'width .2s ease', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: 10, borderTop: `1px solid ${border}` }}>
            <div style={{ padding: '10px 10px', borderRadius: 10, background: `${accent}08`, border: `1px solid ${accent}15` }}>
              ...
              <button onClick={handleLogout} style={{ width: '100%', padding: '4px', borderRadius: 6, border: `1px solid ${border}`, background: 'transparent', color: t3, fontSize: 10, cursor: 'pointer', whiteSpace: 'nowrap' }}>Sign out</button>
            </div>
          </div>
        </div>
      </aside>
              ...
              {notifAbiertas && (
                <div style={{ position: 'absolute', top: '110%', right: 0, width: 320, background: s1, border: `1px solid ${border}`, borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.4)', zIndex: 50, overflow: 'hidden' }}>
                  <div style={{ padding: '14px 16px', borderBottom: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontFamily: 'Syne', fontSize: 14, fontWeight: 700, color: t1 }}>Notifications</div>
                    <button onClick={() => { supabase.from('notificaciones').update({ leida: true }).eq('leida', false).then(() => setNotificaciones(prev => prev.map(n => ({ ...n, leida: true })))) }} style={{ fontSize: 11, color: t3, background: 'none', border: 'none', cursor: 'pointer' }}>Mark all as read</button>
                  </div>
