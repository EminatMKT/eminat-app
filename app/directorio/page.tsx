...
const DIRECTORIO: Miembro[] = [
  ...
  { nombre: 'Maria Jose Malaguera', nickname: 'Majito', cargo: 'Accounting and Revenue Operations Lead', email: 'majo@eminat.net', ubicacion: 'Ecuador', departamento: 'Finance & Admin', empresa: 'Eminat Holding', color: '#7C6FF7' },
  ...
  { nombre: 'Felipe Beltran', cargo: 'Psychiatry', email: 'fbeltran@vivinegretefoundation.org', ubicacion: 'USA', departamento: 'VNF Team', empresa: 'Vivi Negrete Foundation', color: '#FB923C' },
...
function getFlag(ubicacion: string): string {
  if (ubicacion === 'USA') return 'US'
  if (ubicacion === 'Ecuador') return 'EC'
  return ubicacion
}
...
export default function DirectorioPage() {
  ...
  const filtrados = DIRECTORIO
    .filter((m: Miembro) => filtro === 'Todos' || m.departamento === filtro)
    .filter((m: Miembro) =>
      busqueda === '' ||
  ...
  const porDepartamento = DEPARTAMENTOS.slice(1).reduce((acc: Record<string, Miembro[]>, dep: string) => {
    const miembros = filtrados.filter((m: Miembro) => m.departamento === dep)
      ...
      <main className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {Object.entries(EMPRESA_COLORS).map(([empresa, color]: [string, string]) => (
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {DEPARTAMENTOS.map((dep: string) => (
              ...
              >
                {dep}
                {dep !== 'Todos' && (
                  <span className="ml-1 opacity-60">{DIRECTORIO.filter((m: Miembro) => m.departamento === dep).length}</span>
                )}
              </button>
            ))}
          </div>
          ...
          {filtro === 'Todos' ? (
            <div className="space-y-10">
              {Object.entries(porDepartamento).map(([dep, miembros]: [string, Miembro[]]) => (
                <section key={dep}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {miembros.map((m: Miembro, i: number) => (
                      <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between mb-4">
                          ...
                          <span
                            ...
                            style={{ background: `${EMPRESA_COLORS[m.empresa] || '#7C6FF7'}20`, color: EMPRESA_COLORS[m.empresa] || '#7C6FF7' }}
                          >
                        ...
                        <div className="mb-3">
                          <p className="font-semibold text-gray-900 text-sm">
                            {m.nombre}
                            {m.credenciales && <span className="text-gray-400 font-normal text-xs ml-1">{m.credenciales}</span>}
                          </p>
                          {m.nickname && <p className="text-xs text-gray-400 mt-0.5">"{m.nickname}"</p>}
                          <p className="text-xs text-gray-500 mt-1.5">{m.cargo}</p>
                        </div>
                        <div className="space-y-1.5 pt-3 border-t border-gray-50">
                          <a href={`mailto:${m.email}`} className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 truncate">
                            <span>email:</span>
                            <span className="truncate">{m.email}</span>
                          </a>
                          <p className="text-xs text-gray-400">{getFlag(m.ubicacion)} {m.ubicacion}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtrados.map((m: Miembro, i: number) => (
                <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div
                      className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
