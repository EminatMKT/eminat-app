...
export default function AdminPage() {
  ...
  // Delete modal — phase 1 (Cancel / Deactivate / Hard delete)
  const [modalEliminar, setModalEliminar] = useState<string | null>(null)
  const [deleteError, setDeleteError] = useState<string | null>(null)
  const [deleting, setDeleting] = useState(false)
  // Delete modal — phase 2 (Reassign & delete, triggered when hard-delete
  // hits a FK constraint). When non-null, the modal renders the reassign UI.
  ...
  async function eliminarUsuario(id: string) {
    ...
    try {
      ...
      const result = await res.json()
      if (!res.ok || !result.ok) {
        // If the failure is a FK constraint, surface the count of blocking
        // tasks and flip the modal to the reassign-and-delete flow so the
        // admin can transfer ownership instead of just being blocked.
  ...
  async function ejecutarHerencia(oldId: string) {
    ...
    setDeleteError(null)
    if (!reassignState.heirId) {
      setDeleteError('Selecciona quién hereda las tareas.')
      return
    }
    const heir = adminUsuarios.find(u => u.id === reassignState.heirId)
    if (!heir) {
      setDeleteError('Nuevo dueño no encontrado.')
      return
    }
    if (!heir.responsable_ref) {
      setDeleteError(`${heir.nombre} ${heir.apellido} no tiene responsable_ref configurado y no puede heredar tareas.`)
      return
    }
    setDeleting(true)
    try {
      const res = await fetch('/api/admin/reassign-and-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          oldId,
          newId: heir.id,
          newRef: heir.responsable_ref,
          statusOverride: reassignState.statusOverride || null,
        }),
      })
      const result = await res.json()
      if (!res.ok || !result.ok) {
        setDeleteError(result.error || 'No se pudo heredar y borrar.')
        setDeleting(false)
        return
      }
  ...
  return (
    <AppShell actions={crearBtn}>
      ...
      {modalEliminar && (() => {
        ...
        const closeModal = () => { setModalEliminar(null); setDeleteError(null); setReassignState(null) }

        // ── PHASE 2: Reassign & delete (triggered after a FK-blocked hard delete)
        if (reassignState) {
            ...
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
              <div style={{ background: s1, border: `1px solid ${border}`, borderRadius: 18, padding: 28, width: 520, maxWidth: '95vw', maxHeight: '92vh', overflowY: 'auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  ...
                  <button onClick={closeModal} style={{ background: 'none', border: 'none', color: t3, fontSize: 20, cursor: 'pointer' }}>✕</button>
                </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <select
                    value={reassignState.heirId}
                    onChange={e => setReassignState(s => s && { ...s, heirId: e.target.value })}
                    style={inputStyle}
                  >
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 11, color: t3, display: 'block', marginBottom: 5 }}>
                    Estado nuevo (aplica a TODAS las tareas heredadas)
                  </label>
                  <select
                    value={reassignState.statusOverride}
                    onChange={e => setReassignState(s => s && { ...s, statusOverride: e.target.value as StatusOverride })}
                    style={inputStyle}
                  >
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={closeModal} disabled={deleting} style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1px solid ${border}`, background: 'transparent', color: t2, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
                  <button
                    onClick={() => ejecutarHerencia(target.id)}
                    disabled={!heirReady || deleting}
                    style={{ flex: 2, padding: '10px', borderRadius: 10, border: 'none', background: heirReady ? '#F87171' : '#9CA3AF', color: 'white', fontSize: 13, fontWeight: 600, cursor: heirReady ? 'pointer' : 'not-allowed' }}
                  >
        ...
        // ── PHASE 1: Cancel / Deactivate (safer) / Hard delete
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
