      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        ...
        {tab === 'leads' && (
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => { setNewLead({}); setModalNewLead(true) }} style={{ padding: '6px 14px', borderRadius: 8, background: accent, color: 'white', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ New Lead</button>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
            {[
              ...
              { label: 'Active Leads', value: activeLeads, color: '#34D399' },
              { label: 'Awarded', value: awarded, color: '#FBB040' },
              { label: 'In Negotiation', value: inNeg, color: '#F87171' },
            ].map(k => (
                  ...
                  <tr key={l.id} style={{ borderBottom: `1px solid ${border}` }}>
                    ...
                    <td style={{ padding: '7px 10px' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        ...
                        <button onClick={() => { setNewLead(l); setEditingLead(l); setModalNewLead(true) }} style={{ padding: '3px 6px', borderRadius: 6, border: `1px solid ${border}`, background: 'transparent', color: t2, fontSize: 10, cursor: 'pointer' }} title="Edit">✏️</button>
                ...
                {leads.filter(l => !nlSearch || (l.contact_name || '').toLowerCase().includes(nlSearch.toLowerCase()) || (l.email || '').toLowerCase().includes(nlSearch.toLowerCase()) || (l.phone || '').includes(nlSearch)).map(l => (
                  <label key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderBottom: `1px solid ${border}`, cursor: 'pointer' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 10, color: t3 }}>{l.email} · {l.phone || 'No phone'}</div>
                    </div>
            ...
            <div style={{ background: s1, border: `1px solid ${border}`, borderRadius: 14, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={async () => {
                  ...
                  setNlStep(3)
                }} style={{ padding: '10px 24px', borderRadius: 10, background: '#34D399', color: 'white', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Send campaign ✓</button>
              </div>
            </div>
          )}
          {nlStep === 3 && (
            ...
            <div style={{ background: s1, border: `1px solid ${border}`, borderRadius: 14, padding: 16, flex: 1, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                ...
                <button onClick={async () => {
                  ...
                  setSmsMessage('')
                  setSmsSelected([])
                }} disabled={smsSelected.length === 0 || !smsMessage} style={{ padding: '8px 20px', borderRadius: 8, background: smsSelected.length > 0 && smsMessage ? '#34D399' : t3, color: 'white', border: 'none', fontSize: 12, fontWeight: 600, cursor: smsSelected.length > 0 && smsMessage ? 'pointer' : 'default' }}>Send SMS</button>
              </div>
            </div>
            <div style={{ background: s1, border: `1px solid ${border}`, borderRadius: 14, padding: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              {campaigns.filter(c => c.tipo === 'SMS').length === 0 && <div style={{ color: t3, fontSize: 11, textAlign: 'center', padding: 20 }}>No history</div>}
            </div>
          </div>
        </div>
      )}
        ...
        return (
          <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <button onClick={openNewCampaign} style={{ padding: '7px 16px', borderRadius: 8, background: accent, color: 'white', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>+ New campaign</button>
            </div>

            {/* KPIs */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 16 }}>
              {[
                { label: 'Total Campaigns', value: emailCampaigns.length, color: accent },
                { label: 'Sent', value: emailCampaigns.filter(c => c.estado === 'Enviado').length, color: '#34D399' },
                { label: 'Drafts', value: emailCampaigns.filter(c => c.estado === 'Borrador').length, color: '#FBB040' },
                { label: 'Emails Sent', value: totalEnviados, color: '#60A5FA' },
                { label: 'Opened', value: totalAbiertos, color: '#F472B6' },
              ].map(k => (
                  ...
                  <tr key={c.id} style={{ borderBottom: `1px solid ${border}` }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        ...
                        {c.estado === 'Borrador' && <button onClick={() => openEditCampaign(c)} style={{ padding: '3px 8px', borderRadius: 6, fontSize: 10, border: '1px solid rgba(124,111,247,.3)', background: 'transparent', color: accent, cursor: 'pointer' }}>Edit</button>}
                        ...
                        {c.estado === 'Borrador' && <button onClick={() => { openEditCampaign(c); setMailStep(1) }} style={{ padding: '3px 8px', borderRadius: 6, fontSize: 10, border: '1px solid rgba(52,211,153,.3)', background: 'transparent', color: '#34D399', cursor: 'pointer', fontWeight: 600 }}>Send</button>}
                      </div>
                    </td>
                  </tr>
                ))}</tbody>
        ...
        return (
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
              {[
                { label: 'Pipeline Total', value: totalPipeline, color: '#60A5FA' },
                { label: 'Total Awarded', value: totalAwarded, color: '#34D399' },
                { label: 'Estimated Value', value: `$${totalEstimado.toLocaleString()}`, color: '#FBB040' },
              ].map(k => (
                <div key={k.label} style={{ background: s1, border: `1px solid ${border}`, borderRadius: 14, padding: '16px 18px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                  <div style={{ fontSize: 9, color: t3, textTransform: 'uppercase', fontFamily: 'DM Mono', marginBottom: 6 }}>{k.label}</div>
                  <div style={{ fontFamily: 'Syne', fontSize: 28, fontWeight: 800, color: k.color }}>{k.value}</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setModalLead(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: s1, border: `1px solid ${border}`, borderRadius: 18, padding: 28, width: 600, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ borderTop: `1px solid ${border}`, paddingTop: 14, marginBottom: 14 }}>
              ...
              {activities.filter(a => a.lead_id === modalLead.id).length === 0 && <div style={{ fontSize: 11, color: t3 }}>No activities recorded</div>}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => { setNewLead(modalLead); setEditingLead(modalLead); setModalLead(null); setModalNewLead(true) }} style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1px solid ${accent}`, background: 'transparent', color: accent, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>✏️ Edit</button>
              <button onClick={() => { setModalLead(null); setModalActivity(modalLead) }} style={{ flex: 1, padding: '10px', borderRadius: 10, background: '#60A5FA', color: 'white', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>📞 Activity</button>
              <button onClick={() => { if (confirm('Delete this lead?')) deleteLead(modalLead.id) }} style={{ padding: '10px 16px', borderRadius: 10, background: '#F8717120', color: '#F87171', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>🗑</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalNewLead && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => { setModalNewLead(false); setEditingLead(null) }}>
          <div onClick={e => e.stopPropagation()} style={{ background: s1, border: `1px solid ${border}`, borderRadius: 18, padding: 28, width: 600, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setModalNewLead(false); setEditingLead(null) }} style={{ flex: 1, padding: '10px', borderRadius: 10, border: `1px solid ${border}`, background: 'transparent', color: t2, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => saveLead(newLead)} style={{ flex: 2, padding: '10px', borderRadius: 10, background: accent, color: 'white', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>{editingLead ? 'Save changes' : 'Create lead'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setModalActivity(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: s1, border: `1px solid ${border}`, borderRadius: 18, padding: 28, width: 440, maxWidth: '95vw' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={saveActivity} style={{ flex: 2, padding: '10px', borderRadius: 10, background: accent, color: 'white', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Save</button>
            </div>
          </div>
        </div>
      )}
            ...
            {!importPreview ? (
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button onClick={confirmImport} style={{ flex: 2, padding: '10px', borderRadius: 10, background: '#34D399', color: 'white', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Confirm import ({importPreview.length})</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: s1, border: `1px solid ${border}`, borderRadius: 18, padding: 28, width: 680, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {mailStep === 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setModalMailCampaign(false)} style={{ padding: '10px 20px', borderRadius: 10, border: `1px solid ${border}`, background: 'transparent', color: t2, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
                  <div style={{ flex: 1 }} />
                  <button onClick={async () => {
                    await (async () => {
                  ...
                  }} style={{ padding: '10px 20px', borderRadius: 10, border: `1px solid ${border}`, background: 'transparent', color: t1, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Save draft</button>
            ...
            {mailStep === 1 && (
              <div>
                <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center' }}>
                  ...
                  <button onClick={() => { const allIds = leads.filter(l => l.email).map(l => l.id); setMailRecipients(allIds) }} style={{ padding: '8px 14px', borderRadius: 8, border: `1px solid ${border}`, background: 'transparent', color: t2, fontSize: 11, cursor: 'pointer', whiteSpace: 'nowrap' }}>Select all</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setMailViewCampaign(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: s1, border: `1px solid ${border}`, borderRadius: 18, padding: 28, width: 600, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
              {[
                { label: 'Status', value: mailViewCampaign.estado, color: ({ Borrador: '#9CA3AF', Programado: '#60A5FA', Enviado: '#34D399', Cancelado: '#F87171' } as any)[mailViewCampaign.estado] || t3 },
                { label: 'Sent', value: mailViewCampaign.total_enviados || 0, color: '#60A5FA' },
                { label: 'Opened', value: mailViewCampaign.total_abiertos || 0, color: '#34D399' },
            ...
            {mailViewCampaign.fecha_envio && <div style={{ marginBottom: 16 }}><div style={{ fontSize: 11, color: t3, marginBottom: 4 }}>Sent date</div><div style={{ fontSize: 13, color: t1 }}>{new Date(mailViewCampaign.fecha_envio).toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</div></div>}
            {/* Email Preview */}
