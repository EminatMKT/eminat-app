...
export function useResearchData() {
  ...
  const sponsorData = Object.entries(exceptOwn('sponsor').reduce((m: Record<string, number>, l) => { if (l.lead_sponsor) { m[l.lead_sponsor] = (m[l.lead_sponsor] || 0) + 1 } return m }, {}))
    .map(([name, value]) => ({ name, value: value as number }))
    .sort((a, b) => b.value - a.value).slice(0, 8)

  const countryData: Record<string, number> = exceptOwn('country').reduce((m: Record<string, number>, l) => {
