    ...
    <tr style={{ borderBottom: `1px solid ${border}` }}>
      ...
      <td style={{ padding: '10px 14px' }}>
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          ...
          {!isProtected && <button onClick={() => toggleActivo(u.id, !!u.activo)} style={{ padding: '3px 8px', borderRadius: 7, fontSize: 10, border: '1px solid rgba(251,176,64,.3)', background: 'transparent', color: '#FBB040', cursor: 'pointer' }}>{u.activo ? 'Deactivate' : 'Activate'}</button>}
          {!isProtected && <button onClick={() => onDelete(u.id)} style={{ padding: '3px 8px', borderRadius: 7, fontSize: 10, border: '1px solid rgba(248,113,113,.3)', background: 'transparent', color: '#F87171', cursor: 'pointer' }}>Delete</button>}
        </div>
      </td>
    </tr>
  )
