    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={async () => { if (await copyToClipboard(pwd)) mostrarMensaje('ok', t('admin.pwdCopied')) }} style={{ padding: '10px 14px', borderRadius: 10, border: `1px solid ${border}`, background: s2, color: t2, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' }}>📋 {t('common.copy')}</button>
        </div>
      </div>
