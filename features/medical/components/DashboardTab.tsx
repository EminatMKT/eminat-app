    ...
    <div>
      <StaggerGrid style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        <StaggerItem style={statCardStyle}>
          <div style={{ fontSize: 10, color: t3, fontFamily: 'DM Mono', textTransform: 'uppercase', letterSpacing: '.05em' }}>{t('med.statActivePatients')}</div>
          <div style={{ fontSize: 28, fontWeight: 800, fontFamily: 'Syne', color: '#34D399' }}><AnimatedNumber value={pacientesActivos.length} /></div>
          <div style={{ fontSize: 10, color: t3 }}>{pacientes.length} {t('med.statTotal')}</div>
        </StaggerItem>
        <StaggerItem style={statCardStyle}>
          <div style={{ fontSize: 10, color: t3, fontFamily: 'DM Mono', textTransform: 'uppercase', letterSpacing: '.05em' }}>{t('med.statApptsToday')}</div>
          <div style={{ fontSize: 28, fontWeight: 800, fontFamily: 'Syne', color: '#60A5FA' }}><AnimatedNumber value={citasHoy.length} /></div>
          <div style={{ fontSize: 10, color: t3 }}>{citasManana.length} {t('med.statTomorrow')}</div>
        </StaggerItem>
        <StaggerItem style={statCardStyle}>
          ...
          <div style={{ fontSize: 10, color: t3 }}>{t('med.statHipaaCompliance')}</div>
        </StaggerItem>
        <StaggerItem style={statCardStyle}>
          <div style={{ fontSize: 10, color: t3, fontFamily: 'DM Mono', textTransform: 'uppercase', letterSpacing: '.05em' }}>{t('med.statOpenIncidents')}</div>
          ...
          <div style={{ fontSize: 10, color: t3 }}>{incidentes.length} {t('med.statTotal')}</div>
        </StaggerItem>
      </StaggerGrid>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div style={cardStyle}>
          ...
          {citasHoy.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 24, color: t3, fontSize: 12 }}>{t('med.noApptsToday')}</div>
        ...
        <div style={cardStyle}>
          <div style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 14, color: t1, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            🛡️ {t('med.hipaaAlerts')}
            {(incidentesAbiertos.length > 0 || trainingsPendientes.length > 0) && <Badge color="#F87171">{incidentesAbiertos.length + trainingsPendientes.length} {t('med.pendingSuffix')}</Badge>}
          </div>
          {incidentesAbiertos.map(i => <IncidentAlertCard key={i.id} incidente={i} />)}
          {trainingsPendientes.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: t1, marginBottom: 8 }}>{t('med.pendingTraining')}</div>
          ...
          {incidentesAbiertos.length === 0 && trainingsPendientes.length === 0 && (
            <div style={{ textAlign: 'center', padding: 24 }}>
              ...
              <div style={{ fontSize: 12, color: '#34D399', fontWeight: 600 }}>{t('med.allClear')}</div>
            </div>
          )}
        </div>
      </div>
