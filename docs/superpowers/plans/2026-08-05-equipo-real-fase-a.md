...
- **DECIDIDO 2026-08-05 — cargos a N:N en A.2 (NO en este plan).** Este plan (Fase A) construye cargo **único** por persona (`usuarios.cargo_id`) — se deja tal cual, ya probado. **A.2 arranca migrando a N:N** (tabla `usuario_cargos`, split de los cargos con `&` en granulares, migrar `cargo_id`→puente, dropear `cargo_id`) ANTES de escribir el CRUD, para no construir la asignación de cargos dos veces (multiselección por usuario). Motivo: personas con >1 cargo real → trazabilidad. Ver spec §4 (bullet Fase A.2).
- **Directorio (Fase D, post-C):** de-hardcodear `/directorio` (`DIRECTORIO_DATA`/`CARGOS_DIR`) y consolidarlo con el Team tab (misma fuente + componente compartido). NO se toca en la Fase A. Ver spec §4 (bullet Fase D).
